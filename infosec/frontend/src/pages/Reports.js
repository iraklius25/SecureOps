...
export function Reports() {
  ...
  return (
    <div>
      ...
      <div className="tabs">
        {['executive','ale'].map(t => <button key={t} className={`tab-btn ${tab===t?'active':''}`} onClick={()=>setTab(t)}>{t==='ale'?'ALE Breakdown':'Executive Summary'}</button>)}
      </div>
      ...
      {tab === 'ale' && ale && (
        <div>
          <div className="stat-card" style={{marginBottom:16,display:'inline-block',minWidth:220}}>
